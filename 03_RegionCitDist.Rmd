...
#Helper functions
Used later in code for data analysis and plotting. 
```{r}
...
are read at enough depth to call a citrulline if it were citrullinated. 
```{r}
Ags <- c('ra33', 'fiba', 'fibb', 'vim', 'pad4')
...
for (Ag in 1:5){ #for each antigen
  for(pad in 1:2){ #testing pad2 or pad4 cit sites 
    ...
    for(d in 1){ #For each direction mode (actually just bidir. Set 1:3 for all settings. )
      ...
      exp <- colSums(d_exp, na.rm = TRUE) / N 
...
ggplot(df1[-which(df1$ag == "pad4"),], aes(fill = cit, x = type)) + 
  ...
  facet_grid(~ cond)  
```
#Violin plot of distance to cit for created and destroyed regions.----
This data comprises figure 2C. 
```{r}

 ggplot(brdf[-which(brdf$ag == 'pad4'),], aes(x = type, y = diff)) + 
    geom_violin() + 
    theme_prism() + 
